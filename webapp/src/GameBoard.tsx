...
const API_GATEWAY_URL = import.meta.env.VITE_GATEWAY_URL ?? 'http://localhost:8000';
...
export default function GameBoard({ onBack }: { onBack: () => void }) {
  ...
  const svgWidth =
    boardMargin * 2 + (boardSize - 1) * hexHorizontalSpacing + hexHorizontalSpacing;
          ...
          >
            {Array.from({ length: boardSize }, (_, row) =>
              Array.from({ length: row + 1 }, (_, col) => {
                ...
                const cy =
                  boardMargin + row * hexVerticalSpacing + hexRadius;
