...
export default function GameBoard({ onBack }: { onBack: () => void }) {
  ...
  const SY = R * 1.5;
  const M  = R * 3;
  const svgW = M * 2 + (n - 1) * SX + SX;
  const svgH = M * 2 + (n - 1) * SY + R * 2;
  ...
  const handleCellClick = useCallback(async (x: number, y: number, z: number) => {
    ...
    // ── 2. Ask bot for its move via gateway ──────────────────────────────────
    setCurrentTurn(1);
    setLoading(true);
  ...
  }, [boardMap, currentTurn, gameStatus, loading, n]);
          ...
          >
            ...
            {Array.from({ length: n }, (_, row) =>
              Array.from({ length: row + 1 }, (_, col) => {
                ...
                const cx = M + ((n - 1 - row) * SX / 2) + col * SX + SX / 2;
                const cy = M + row * SY + R;
